...
void print_results_raw(const arma::mat &results)
{
    ...
    for (size_t i = 0; i < results.n_rows; i++)
        for (size_t j = 0; j < results.n_cols; j++)
            printf("  [ %3d , %3d ]: %24.18lf\n", i + 1, j + 1, results(i, j));
            // TODO set same field widths/precision as printf call above.
...
void print_results_with_labels(
    ...
    for (size_t i = 0; i < results.n_rows; i++)
        for (size_t j = 0; j < results.n_cols; j++)
            // printf("  [ %3d : %10s %10s , %3d : %10s %10s ]: %24.18lf\n",
            //        i + 1, labels_1[i].c_str(), labels_2[i].c_str(),
            //        j + 1, labels_1[j].c_str(), labels_2[j].c_str(),
            //        results(i, j));
            // TODO set same field widths/precision as printf call above.
            std::cout << "  [ " << i + 1 << " : " << labels_1[i] << " " << labels_2[i]
                      << " , " << j + 1 << " : " << labels_1[j] << " " << labels_2[j]
                      << " ]: " << results(i, j) << std::endl;
...
void print_polarizability(std::ostringstream &os, const arma::mat &polar_tensor)
{
    ...
    // os << "  Isotropic polarizability   [a.u.] " << std::setw(14) << (arma::trace(polar_tensor) / 3.0) << std::endl;
    // os << "  Anisotropic polarizability [a.u.] " << std::setw(14) << _calc_matrix_anisotropy(polar_tensor) << std::endl;
    ...
    // os << "  Isotropic polarizability   [angstrom^3] " << std::setw(14) << (arma::trace(polar_tensor) / 3.0) * au_to_cubic_angstrom << std::endl;
    // os << "  Anisotropic polarizability [angstrom^3] " << std::setw(14) << _calc_matrix_anisotropy(polar_tensor) * au_to_cubic_angstrom << std::endl;
...
void printf_14_7_1by3(const arma::vec &v) {
    if (v.n_elem != 3)
        throw std::runtime_error("v.n_elem != 3");
    printf("            %14.7f %14.7f %14.7f\n",
           v(0), v(1), v(2));
}
